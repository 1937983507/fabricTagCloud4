...
- POI数据导入模块，支持用户导入自己的POI数据。
- 数据筛选模块，从地图模块中抽离出来，支持文本框输入“中心地点”与“半径”(或“POI数量”)，然后自动进行POI数据的筛选。
- 颜色选择模块，目前有些色带是倒序的，希望可以统一整理一下
- 颜色选择模块，是否可以更加的智能，推荐用户选择较好的色带？
- 兼容移动端布局，但是移动端下可能就难以使用手进行地图范围的绘制，此时就可以使用文本框进行。
