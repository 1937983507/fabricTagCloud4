...
- **API Key 配置错误**：高德地图 API Key 配置不正确
  - **解决方案**：检查项目根目录 `.env` 中的 `VITE_AMAP_KEY`（及可选的 `VITE_AMAP_SECURITY_JS_CODE`），并在高德控制台核对 **安全密钥（jscode）** 与 **Referer 白名单**
...
1. 补充算法窗口，支持对各布局算法的参数进行调节。

### 2026-04-11

1. 将帮助页面也适配移动端展示。
2. 修复子页面切换时绘制的标签云丢失问题。
3. 修复页面窗口变化导致标签云画布保持初始化大小。
4. 修复标签页面缩放下导致的页面控件挤压问题。

### 2026-04-10

1. 修复导出SVG时图幅坐标锚点偏移的问题。
2. 导出SVG时动态图例大小，使其适配画布。
3. 移动端字体库采用另一套标准。

### 2026-04-08

1. 图例部分优先尝试整数展示，若有重叠再采用保留一位小数。

### 2026-04-07

1. 创建 .env 文件存储 js key 等配置文件。
2. 周边筛选下，生成的圆形覆盖物，支持编辑（编辑圆心与半径）。
3. 周边筛选下，若是文本输入，则优先从系统内部进行点匹配，匹配失败再走高德地理编码。
4. 优化背景色、色带、单复色修改下颜色修改的性能。
5. 支持对当前色带中的每一色块进行定制修改。
6. 导入数据窗口下，添加对示例数据的下载。
7. 修复自然间断点算法存在的问题。
