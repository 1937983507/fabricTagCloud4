...
![算法详情](../../../public/img/2AlgorithmDetails.png "scale:95%")
...
![多尺度模型](../../../public/img/7multiscaleModel.png "scale:95%")

#### 2.3 实时可视化参数联动

系统采用 **Vue 3 Composition API + Pinia 状态管理** 架构，实现多面板参数的实时同步与标签云画布的即时渲染：

- **字体面板**：字体类别数量、字号大小、字重、字体类型
- **配色面板**：背景色与文字色自定义、色带配置、配色数量、渲染方式（分位数、相等间隔、几何间隔、自然间断点、标准差等）
- **数据管理**：POI 数据筛选、编辑、导入、导出

所有参数修改均通过 Pinia Store 统一管理，确保多组件间的数据一致性。

![标签云绘制](../../../public/img/3tagCloudDrawing.png "scale:95%")

#### 2.4 数据定制化导入

系统内置有全国范围内的**景点兴趣点数据**，同时支持用户导入**自定义兴趣点数据**，拓展绘制内容的范围。用户可以根据自身需求以及所处行业的特点，生成具有更高定制性和适用性的标签云，以更好地满足不同用户群体的需求。

![数据导入](../../../public/img/4dataImport.png "scale:95%")
...
![字体修改](../../../public/img/5fontModification.png "scale:95%")

![配色修改](../../../public/img/6ColourModification.png "scale:95%")
...
![初始界面](../../../public/img/1initialScreen.png "scale:95%")

### 2. **查看兴趣点数据**

可以通过**地图**查看目前系统内存储的**兴趣点数据**。系统初始界面将会展示用户所在定位的**高德地图**，并以**点覆盖物**的形式展示全国范围内的**景点POI数据**。

![地图查看](../../../public/img/2mapView.png "scale:45%")

### 3. **兴趣点数据筛选**

可以对其中的数据进行筛选，并通过**二维表**查看目前筛选的数据。用户通过地图显示模块的**数据筛选**功能，在地图上绘制不同的**面状覆盖物**（圆形、矩形、椭圆形、多边形），即可在系统中筛选对应的点数据。

![数据筛选](../../../public/img/3dataFiltering.png "scale:95%")

### 4. **生成地名标签云**

单击**运行生成标签云**之后，系统将会对用户所框选的兴趣点数据进行筛选，并采用**分位数法**对其字号、颜色进行设置，接着使用内置的**压盖检测与多角度径向偏移策略**绘制不重叠且基本无偏的地名标签云。

![生成标签云](../../../public/img/4tagCloudDrawing.png "scale:95%")

### 5. **字体修改**

在**字体**栏目可以对当前的标签云的字体属性进行更改，包括**字号、字重、字体**属性。其每一修改，将会对已绘制的地名标签云进行**实时更新**。
...
![地图视图](../../../public/img/1MapView.png "scale:95%")
...
- **数据编辑**：
  ...
  - 用户可以输入地名，对数据记录进行添加（需要处于数据可编辑状态）
...
![数据编辑](../../../public/img/6dataEditing.png "scale:95%")
...
在**数据导入**窗口，需要用户指定：
- **文件名**：选择要导入的数据文件
- **数据编码**：指定文件编码格式（如 UTF-8、GBK 等）
- **文本分隔符**：指定 CSV 文件的分隔符（逗号、制表符等）
- **数据坐标系**：指定数据的坐标系（WGS84、GCJ02 等）

在系统读取首行字段之后，用户还需要进行**字段匹配**，包括：
- **地名**：兴趣点名称字段
- **经度**：经度坐标字段
- **纬度**：纬度坐标字段
- **量值字段**：例如景点的评分数等定量指标

接着，便可在窗口下方进行**数据预览**。在用户检查字段匹配之后，即可单击**导入数据**将文件中所有兴趣点数据导入系统。

![数据导入](../../../public/img/10dataImport.png "scale:95%")
...
- **圆形覆盖物**：以圆心和半径定义圆形区域
- **矩形覆盖物**：以对角两点定义矩形区域
- **椭圆形覆盖物**：以中心点和长短轴定义椭圆区域
- **多边形覆盖物**：以多个顶点定义任意多边形区域

同时也支持用户通过**清除绘制**将所有的覆盖物与已筛选数据进行清除。

![数据筛选](../../../public/img/11dataFiltering.png "scale:95%")
...
![多尺度模型](../../../public/img/14multiscaleModel.png "scale:95%")
...
![改变字体](../../../public/img/15changeFont.png "scale:95%")
...
- **数据格式错误**：导入的数据格式不正确或字段匹配错误
  - **解决方案**：检查数据格式，确保包含地名、经度、纬度等必需字段，并正确匹配字段
...
**可能原因与解决方案**：
- **文件格式错误**：文件格式不是 CSV 或编码格式不正确
  - **解决方案**：确保文件为 CSV 格式，编码为 UTF-8 或 GBK
- **字段匹配错误**：字段匹配不正确
  - **解决方案**：仔细检查字段匹配，确保地名、经度、纬度等必需字段正确匹配
